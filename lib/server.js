  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        try {
          raw = txp.getRawTx();
          console.log('txp.getRawTx', raw);
        } catch (ex) {
          return cb(ex);
        }
        console.log('opts.proposalSignature', opts.proposalSignature);
        console.log('copayer.requestPubKeys', copayer.requestPubKeys);
  ...
  function decorate(wallet, txs, addresses, proposals, notes) {
    ...
    return _.map(txs, function(tx) {
      ...
      if (tx.outputs.length || tx.inputs.length) {
        ...
        if (tx.outputs[0].value == 0 && tx.outputs[0].script.length == 0) {
          amount = amountOut - amountIn;
          action = 'staked';
        } else if (amountIn == (amountOut + amountOutChange + (amountIn > 0 ? tx.fees : 0))) {
          amount = amountOut;
          action = 'moved';
        } else {
        ...
        if (action == 'sent' || action == 'moved' || action == 'staked') {
...
/**
 * Gets the sync status of the blockchain as well as the network's blockheight and this node's current block
 * @param {Object} opts
 * @returns {Object}
 */
WalletService.prototype.getSyncStatus = function(cb) {
  var self = this;
  var bc = self._getBlockchainExplorer('livenet');
  bc.getBlockchainSyncInfo(function(err, statusInfo) {
    if (err) return cb(err);
